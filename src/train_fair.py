        ...
        if run is not None:
            ...
            for r in result:
                if self.metric:
                    ...
                    index = np.argmin(r[:, 10])
                best_epoch.append(index)
                valid_f1 = r[index, 1].item()
                test_f1 = r[index, 2].item()
                valid_auc = r[index, 4].item()
                test_auc = r[index, 5].item()
                test_parity = r[index, 6].item()
                test_equ = r[index, 7].item()
                test_cf = r[index, 8].item()
                best_results.append((valid_f1, test_f1, valid_auc, test_auc, test_parity, test_equ, test_cf))
            ...
            r = best_result[:, 2]
            print(f'Final Valid AUC: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 3]
            print(f'Final Test AUC: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 4]
            print(f'Final Test Parity: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 5]
            print(f'Final Test Equ: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 6]
            print(f'Final Test cf: {r.mean():.2f} ± {r.std():.2f}')
            return best_result
...
def fair_metric(pred, labels, sens):
    idx_s0 = sens==0
    idx_s1 = sens==1
    idx_s0_y1 = list(set(np.where(idx_s0)[0])&set(np.where(labels==1)[0]))
    idx_s1_y1 = list(set(np.where(idx_s1)[0])&set(np.where(labels==1)[0]))
    parity = abs(sum(pred[idx_s0])/sum(idx_s0)-sum(pred[idx_s1])/sum(idx_s1))
    equality = abs(sum(pred[idx_s0_y1])/len(idx_s0_y1)-sum(pred[idx_s1_y1])/len(idx_s1_y1))
    return parity.item(), equality.item()

@torch.no_grad()
def evaluate_fair(model, data, split_idx, eval_func, result=None):
    if result is not None:
        out = result
    else:
        model.eval()
    ...
    test_loss = F.nll_loss(
        out[split_idx['test']], data.y[split_idx['test']])
    out = torch.argmax(out,dim=1).type_as(data.y)
    # print(torch.sum(data.y[split_idx['test']]==0), torch.sum(data.y[split_idx['test']]==1))
    # print(torch.sum(data.y[split_idx['train']]==0), torch.sum(data.y[split_idx['train']]==1))
    # print(train_loss, valid_loss)
    train_f1_s = f1_score(data.y[split_idx['train']].cpu().numpy(), out[split_idx['train']].cpu().numpy())
    train_auc_roc = roc_auc_score(data.y[split_idx['train']].cpu().numpy(), prob[split_idx['train']].cpu().numpy())

    valid_f1_s = f1_score(data.y[split_idx['valid']].cpu().numpy(), out[split_idx['valid']].cpu().numpy())
    valid_auc_roc = roc_auc_score(data.y[split_idx['valid']].cpu().numpy(), prob[split_idx['valid']].cpu().numpy())

    
    test_f1_s = f1_score(data.y[split_idx['test']].cpu().numpy(), out[split_idx['test']].cpu().numpy())
    test_auc_roc = roc_auc_score(data.y[split_idx['test']].cpu().numpy(), prob[split_idx['test']].cpu().numpy())
    parity, equality = fair_metric(out[split_idx['test']].cpu().numpy(), data.y[split_idx['test']].cpu().numpy(), sens[split_idx['test']])
    # cf
    ori_x = data.x
    cf_x = copy.deepcopy(ori_x)
    cf_x[:, sens_idx] = 1-cf_x[:, sens_idx]
    data.x = cf_x
    c_out = model(data)
    c_out = F.log_softmax(c_out, dim=1)
    c_out = torch.argmax(c_out,dim=1).type_as(data.y)
    counterfactual_fairness = 1 - (out.eq(c_out)[split_idx['test']].sum().item()/split_idx['test'].shape[0])
    data.x = ori_x
    return train_f1_s, valid_f1_s, test_f1_s, train_auc_roc, valid_auc_roc, test_auc_roc, parity, equality, counterfactual_fairness, train_loss, valid_loss, test_loss
...
if __name__ == '__main__':
    ...
    logger = Logger_fair(args.runs, args)
    ...
    #             scheduler.step()
    #         Evaluation part
            result = evaluate_fair(model, data, split_idx, eval_func)
    ...
    with open(filename, 'a+') as write_obj:
        if args.m_l:
            cur_line = f'fair_{args.method}_{args.m_l}_{args.lr}_{args.wd}_{args.sub_size}_{args.heads}_aug_{args.aug}_ratio_{str(args.aug_ratio)}_t_{str(args.t)}_plr_{str(args.p_lr)}_pepoch_{str(args.p_epochs)}_player_{str(args.p_layer)}_phidden_{str(args.p_hidden)}_drop_{str(args.dropout)}_train_{str(args.train_prop)}'
            if args.add_e:
                cur_line+="_add_e"
        else:
            cur_line = f'fair_{args.method}_{args.lr}_{args.wd}_{args.heads}_{str(args.dropout)}_train_{str(args.train_prop)}'
        cur_line += f',{best[:,0].mean():.3f} ± {best[:,0].std():.3f}'
        cur_line += f',{best[:,1].mean():.3f} ± {best[:,1].std():.3f}'
        cur_line += f',{best[:,2].mean():.3f} ± {best[:,2].std():.3f}'
        cur_line += f',{best[:,3].mean():.3f} ± {best[:,3].std():.3f}'
        cur_line += f',{best[:,4].mean():.3f} ± {best[:,4].std():.3f}'
        cur_line += f',{best[:,5].mean():.3f} ± {best[:,5].std():.3f}'
        cur_line += f',{best[:,6].mean():.3f} ± {best[:,6].std():.3f}'
