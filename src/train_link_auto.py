...
if __name__ == '__main__':
    ...
    # # Part 2: Load model
        ...
        model.train()
        # print('MODEL:', model)
        # with open("../data/AllSet_all_raw_data/yelp/yelp_restaurant_name.csv") as f:
        #     name = f.readlines()
        #     name = [sth.strip() for sth in name]
        #     name = name[1:]
        #     print(name[75])
        ### Training loop ###
    ...
    #         scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=100, gamma=0.51)
        ...
        if args.deg==0:
            ...
            if args.multi:
                encoder2, decoder2 = vhgae_encoder(args).to(device), vhgae_decoder(args).to(device)
                encoder2.reset_parameters()
                view_generator2 = vhgae(encoder2, decoder2, args).to(device)
        elif args.deg==1:
            encoder, decoder = vhae_encoder(args).to(device), vhae_decoder(args).to(device)
            encoder.reset_parameters()
            view_generator = vhae(encoder, decoder, args).to(device)
            if args.multi:
                encoder2, decoder2 = vhae_encoder(args).to(device), vhae_decoder(args).to(device)
                encoder2.reset_parameters()
                view_generator2 = vhae(encoder2, decoder2, args).to(device)
        if args.multi:
            optimizer_g = torch.optim.Adam([
            {'params': view_generator.parameters()}, 
            {'params': view_generator2.parameters()},], lr=args.g_lr, weight_decay=0)
        else:
        ...
        for epoch in range(args.epochs):
            ...
            if args.multi:
                ...
                for sth in range(args.step):
                    ...
                    data_1 = data_sub.to(device)
                    if not args.deg:
                        data_1.edge_index_neg = negative_sampling(data_1.edge_index, num_nodes=[int(data_1.n_x[0].item()), int(data_1.totedges.item())])
                    loss_vhgae1, data_aug1, aug_weight1, drop1 = view_generator.generate(data_1)
                    out = model.forward_cl(copy.deepcopy(data_1).to(device), aug_weight1)
                    ...
                    out_aug = model.forward_cl(copy.deepcopy(data_2).to(device), aug_weight)
                    loss_cl = contrastive_loss_node(out, out_aug, args)
                    if args.deg:
                        loss_generator = torch.square(drop-args.aug_ratio)+torch.square(drop1-args.aug_ratio)-args.g_l*loss_cl
                    else:
                        loss_generator = loss_vhgae+loss_vhgae1-args.g_l*loss_cl
                ...
                for sth in range(args.step):
                    ...
                    data_2 = copy.deepcopy(data_sub).to(device)
                    if not args.deg:
                        n_hy = torch.max(data_2.edge_index[1])+1
                        data_2.edge_index_neg = negative_sampling(data_2.edge_index, num_nodes=[int(data_2.n_x[0].item()), n_hy])
                    loss_vhgae, data_aug2, aug_weight, drop = view_generator.generate(data_2)
                    if args.method=="AllSetTransformer":
            ...
            if args.multi:
                ...
                out = model.forward_cl(data_1, aug_weight1)
                data_2 = copy.deepcopy(data_sub).to(device)
                with torch.no_grad():
                    _, data_aug2, aug_weight, drop = view_generator.generate_only(data_2)
                out_aug = model.forward_cl(data_2, aug_weight)
            else:
                model.train()
                view_generator.eval()
                ...
                if args.method=="AllSetTransformer":
                    ...
                    aug_weight_attn = aug_weight
                # data_sub_nodes = data_sub.sub_nodes.cpu().numpy()
                # sel_num = 16
                # label_cur = data_sub.y
                # print(epoch)
                # if sel_num in data_sub_nodes:

                #     map_index = np.where(data_sub_nodes==sel_num)[0][0]
                #     index_sel = data_sub.edge_index[0]==map_index
                #     index_he_sel = data_sub.edge_index[1][index_sel]
                #     keep_edges = torch.where(aug_weight_attn==1)[0]
                #     with open("../sub_mask_edges_label_yelp.txt","a+") as f:
                #         f.write("\n"+"\n"+"\n"+"\n"+str(epoch)+"\n")
                #     for he_cur in index_he_sel:
                #         he_cur_index = torch.where(data_sub.edge_index[1]==he_cur)[0]
                #         if he_cur_index.shape[0]>1:
                #             nodes_cur = data_sub.edge_index[0][he_cur_index]
                #             nodes_cur = nodes_cur.cpu().numpy()
                #             labels_cur = label_cur[nodes_cur]
                #             ori_nodes = data_sub_nodes[nodes_cur]
                #             keep_cur_index = set(keep_edges.cpu().numpy())&set(he_cur_index.cpu().numpy())
                #             nodes_keep_cur = data_sub.edge_index[0][list(keep_cur_index)]
                #             nodes_keep_cur = nodes_keep_cur.cpu().numpy()
                #             labels_keep_cur = label_cur[nodes_keep_cur]
                #             ori_nodes_keep = data_sub_nodes[nodes_keep_cur]
                #             name_all = [name[ori_node] for ori_node in ori_nodes]
                #             name_keep = [name[ori_node_keep] for ori_node_keep in ori_nodes_keep]
                #             with open("../sub_mask_edges_label_yelp.txt","a+") as f:
                #                 f.write("ori content:"+str(name_all)+str(labels_cur)+"\n"+"keep:"+str(name_keep)+str(labels_keep_cur)+"\n")

                # num_hyperedges_sub = data_sub.num_hyperedges[0].item()
                # keep_hes = scatter(aug_weight_attn, data_sub.edge_index[1], dim=0, reduce="sum").cpu().numpy()[:num_hyperedges_sub]
                # fre = Counter(data_sub.edge_index[1].cpu().numpy())
                # valid_items = list(fre.items())[:num_hyperedges_sub]
                # fre_sort = sorted(valid_items,key=lambda item:item[1],reverse=True)
                # top_hes = [sth[0] for sth in fre_sort[:5]]
                # gt_fre = np.array([sth[1] for sth in fre_sort[:5]])
                # top_drop = np.mean(1-keep_hes[top_hes]/gt_fre)
                # print(epoch, drop, top_drop)

                # with open("../{}_mask_{}.txt".format(args.dname,args.g_l),"a+") as f:
                #     f.write(f'epoch {epoch} drop: {drop} top_drop: {top_drop}\n')
                out_aug = model.forward_cl(copy.deepcopy(data_sub).to(device), aug_weight_attn)
            loss_cl = contrastive_loss_node(out, out_aug, args)
            # with torch.no_grad():
            #     num_ori_edge = data_sub.num_ori_edge
            #     aug_keep_prob = torch.ones(aug_weight.shape).to(device)
            #     r_index = np.random.choice(num_ori_edge, int(drop.item()*num_ori_edge), replace=False)
            #     aug_keep_prob[r_index]=0
            #     random_aug_weight_attn = aug_keep_prob
            #     out_aug_random = model.forward_cl(copy.deepcopy(data_sub).to(device), random_aug_weight_attn)
            #     loss_cl_r = contrastive_loss_node(out, out_aug_random, args)
                # print(drop, loss_cl, loss_cl_r)
            # loss_cl=0
            # sup loss
            if args.linear:
                out = model.forward_finetune(data)
                ...
                x_he, x_node = model.forward_link(data)
                edge_index_neg = negative_sampling(data.edge_index_ori, num_nodes=[int(data.n_x[0].item()), x_he.shape[0]])
                edge_index = data.edge_index
